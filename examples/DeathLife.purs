...
nextGameState maybePress st =
  ...
  in st
      { alive = newAlive
      , justDied = Array.difference st.alive newAlive
      , staging =
          { countdown:
              if didPress
                then newCountDown
  ...
  let filledLife =
      ...
      filledStaging =
        flip map stagingCells $ \(Tuple x y) ->
          unsafeButtonRef x y /\ Just (Color Orange High)
      filledDied =
        flip map diedCells $ \(Tuple x y) ->
          unsafeButtonRef x y /\ Just (Color Yellow Low)
      blank = do
...
renderBoard c st =
  setButtons c $ cellsToButtons st.alive st.staging.cells st.justDied
